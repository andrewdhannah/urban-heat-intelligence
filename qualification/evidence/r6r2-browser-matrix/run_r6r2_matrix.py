#!/usr/bin/env python3
"""R6-R2 browser/consumer proof matrix — frozen candidate 224ef1c417544d9314f2400a6b20ae46d7fba409.

Evidence generation only. Does NOT modify any tracked file. Deterministic route
mocking of /api/answer (and of third-party assets so the run is network-independent):

  * unpkg Leaflet JS/CSS  -> local copies stored under ./lib (byte-identical to the
    upstream 1.9.4 dist files so the SRI on the page's <script> tag still passes).
  * OSM tile images       -> a 1x1 PNG (tile *elements* are what the consumer needs).
  * /api/answer           -> per-obligation mocked payloads (mode-aware).

A page-probe (injected via add_init_script) captures the Leaflet map instance at
marker-creation time so map-fill checks can call map.getSize() directly.

Summary record is written to r6r2-browser-matrix-results.json; key-row screenshots
are written under ./screenshots.

Usage:
    python3 run_r6r2_matrix.py
"""
import asyncio
import base64
import json
...
URL = os.environ.get("R6R2_MATRIX_URL", "http://127.0.0.1:8092/").rstrip("/") + "/"
BUILD = "224ef1c417544d9314f2400a6b20ae46d7fba409"
HEADLESS = os.environ.get("R6R2_MATRIX_HEADLESS", "1") == "1"
...
async def run():
    ...
    summary = {
        "matrix": "R6-R2 browser/consumer proof matrix",
    ...
    out = BASE / "r6r2-browser-matrix-results.json"
...
for _qid, _qtext, _expected in EXPLORE:
    ...
    row(qid, f"Explore question: {qtext}", DESKTOP,
        ...
        evidence=f"Catalogue question '{qtext}' executes on the loaded Replay evidence; the rendered analyst answer is asserted to match the intent the catalogue declares for it (expected pattern: {expected!r}). FAIL rows are genuine intent-routing mismatches in the frozen candidate.")
...
row(20, "Candidate 1 selection (card click → marker focus → source-cell highlight)", ALL4,
    ...
    evidence="PARTIAL DEFECT: card-click→marker-focus (single focused card + elevated marker) and map zoom PASS, but source-cell highlight is INERT at all 4 viewports: heatLayer is Leaflet-Canvas-rendered (preferCanvas:true), each cell layer's getElement() is undefined, so highlightSourceCell() applies no .source-cell-highlight anywhere. Card click → marker focus → (no) source-cell highlight.")
...
async def _impl_candidate_1(browser, vp):
    ...
    if not focus["hasHighlight"]:
        ...
        raise AssertionError(
            "PARTIAL DEFECT — card click → marker focus + zoom PASS, but source-cell highlight is INERT: "
            "heatLayer uses Leaflet's Canvas renderer (preferCanvas:true) so each measured-cell layer's "
            "getElement() is undefined and highlightSourceCell() can add no .source-cell-highlight to any DOM "
            "element (canvas has no per-feature element). CSS/JS highlight path is a no-op. "
            f"focus={focus!r}."
        )
    if vp == (1440, 900):
...
row(26, "Source-cell highlight on focus", DESKTOP,
    ...
    evidence="DEFECT (consumer-visible): focusing a candidate never highlights the source cell. Leaflet is configured preferCanvas:true; measured-cell paths are drawn to a shared canvas with no per-feature DOM element (layer.getElement() === undefined for every cell), so highlightSourceCell() can add no .source-cell-highlight. The CSS highlight contract (.leaflet-interactive.source-cell-highlight) presumes an SVG renderer.")
...
async def _impl_source_cell_highlight(browser, vp):
    ...
    if not focus["hasHighlight"]:
        ...
        raise AssertionError(
            "DEFECT — source-cell highlight does not manifest on focus: with preferCanvas:true the "
            "measured cells have no per-feature DOM element (layer.getElement() === undefined), so "
            "highlightSourceCell() cannot apply .source-cell-highlight. Canvas selection never occurs."
        )
...
row(33, "Mobile: map + Current Read stacking (map above Current Read, compact map)", [(390, 844)],
    ...
    evidence="DEFECT (consumer-visible): with results rendered (body.has-result), `.decision-grid` keeps the desktop two-column track `minmax(0,4fr) minmax(230px,1fr)` because `body.has-result .decision-grid` (dashboard.css, specificity 0,2,1) overrides the responsive single-column `@media(max-width:1050px){.decision-grid{grid-template-columns:1fr}}` rule (0,1,0). At 390px the map and Current Read render side-by-side (132px + 230px columns, rail above, map panel 1221px tall), never stacked; map is NOT compact. Loading state stacks, resolved state does not.")
...
row(35, "Mobile: explore questions collapsed by default", [(390, 844)],
    ...
    evidence="DEFECT (consumer-visible): on mobile the JS sets #catalogue-panel hidden=true (collapse intent), but `.catalogue-panel{display:grid}` (author CSS) overrides the UA `[hidden]{display:none}` — there is no `[hidden]` rule in the stylesheets — so the Explore panel is VISIBLE on initial load at 390x844 (aria-expanded=false but rendered). Toggling also has no visual effect.")
...
row(38, "Replay historical NWS consumer path (HISTORICAL NWS banner with station observation)", [(1440, 900)],
    ...
    evidence="Replay consumer renders the HISTORICAL NWS · REPLAY · NOT USED TO RANK banner with the KPHX station observation (45.0°C · Sunny, station + timestamp + offset, wind/humidity) and active-hazard line; hero rail shows HISTORICAL OBSERVATION with the 45.0°C value. Ancillary observation (same defect family as #35): the app hides #nws-source-line for Replay via the hidden attribute, but `.source-line{display:flex}` overrides UA [hidden], so the NWS source line stays visible in Replay.")
...
async def _impl_replay_nws_consumer(browser, vp):
    ...
    # NOTE: app sets #nws-source-line hidden=true for Replay, but `.source-line{display:flex}`
    # overrides the UA [hidden] rule, so the NWS source line remains visible in Replay.
    # This is the same hidden-attribute defect family as obligation 35; recorded, not a
    # failure of this row's core (the HISTORICAL NWS banner consumer path).
    nws_source_visible = await page.locator("#nws-source-line").is_visible()
    await shot(page, "r38-replay-nws-consumer-1440")
    return nws_source_visible
...
row(39, "Live NWS consumer path (NWS FORECAST banner, supplemental, not used to rank)", [(1440, 900)],
    ...
    evidence="DEFECT (consumer-visible): the Live NWS FORECAST banner is populated with forecast content but then immediately hidden: render() calls renderNwsForecast() (banner.hidden=false) then renderHistoricalNwsContext() which unconditionally sets banner.hidden=true for any non-replay payload. Consumer never sees the banner in Live mode. Thermal ranking itself remains untouched (ranks 1/2/3, 'Candidate 1 leads'). Mocked live payload; provider paths mocked by contract.")
...
async def _impl_live_nws_consumer(browser, vp):
    ...
    assert ranks == ["1", "2", "3"], ranks
    assert "Candidate 1 leads" in await page.locator("#ranking-callout").inner_text()
    assert "Start with candidate 1." in await page.locator("#answer-hero").inner_text()
    await shot(page, "r39-live-nws-banner-hidden-1440")
    if not visible:
        raise AssertionError(
            ...
            f"hero label: {await page.locator('#hero-context-label').inner_text()!r}."
        )
